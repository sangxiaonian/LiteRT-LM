...
void FillMasksInternal(T* mask_local, T* mask_global, int64_t seq_q,
  ...
  if (sizeof(T) == 1) {
    std::memset(mask_local, (int)masked_val, seq_q * seq_k);
    std::memset(mask_global, (int)masked_val, seq_q * seq_k);
  } else {
    for (int64_t i = 0; i < seq_q * seq_k; ++i) {
      mask_local[i] = masked_val;
      mask_global[i] = masked_val;
    }
  }
  ...
  for (int64_t q = 0; q < seq_q; ++q) {
    ...
    T* local_row = mask_local + (q * seq_k);
    T* global_row = mask_global + (q * seq_k);
    ...
    for (int64_t k = 0; k < std::min(kv_valid_limit, kv_cache_capacity); ++k) {
      global_row[k] = valid_val;
      // Sliding window (512 tokens).
      if (k >= effective_pos - 511) {
...
absl::Status HWMaskUpdate(
  ...
  auto& mask_local_buf = out_buffers.at(kMaskLocal);
  auto& mask_global_buf = out_buffers.at(kMaskGlobal);

  LITERT_ASSIGN_OR_RETURN(auto mask_type, mask_local_buf.TensorType());
  ...
  LITERT_ASSIGN_OR_RETURN(
      auto mask_local_lock,
      ::litert::TensorBufferScopedLock::Create(
          mask_local_buf, ::litert::TensorBuffer::LockMode::kWrite));
  LITERT_ASSIGN_OR_RETURN(
      auto mask_global_lock,
      ::litert::TensorBufferScopedLock::Create(
          mask_global_buf, ::litert::TensorBuffer::LockMode::kWrite));

  // Dispatch by Dtype
  if (mask_type.ElementType() == ::litert::ElementType::Int8) {
    FillMasksInternal<int8_t>(static_cast<int8_t*>(mask_local_lock.second),
                              static_cast<int8_t*>(mask_global_lock.second),
                              seq_q, seq_k, time_step, input_tokens,
                              /*valid_val=*/127, /*masked_val=*/-128);
  } else if (mask_type.ElementType() == ::litert::ElementType::Int16) {
    FillMasksInternal<int16_t>(static_cast<int16_t*>(mask_local_lock.second),
                               static_cast<int16_t*>(mask_global_lock.second),
                               seq_q, seq_k, time_step, input_tokens,
