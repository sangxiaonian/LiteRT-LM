...
TEST(LlmExecutorExternalSamplerStaticTest, CorrectnessTest) {
  ...
  for (int i = 0; i < tokens.size(); ++i) {
    ...
    EXPECT_NEAR(logits_span[0], expected_logits[i][0], 1e-3);
    EXPECT_NEAR(logits_span[1], expected_logits[i][1], 1e-3);
    EXPECT_NEAR(logits_span[2], expected_logits[i][2], 1e-3);
    next_position += 1;
  }
}
